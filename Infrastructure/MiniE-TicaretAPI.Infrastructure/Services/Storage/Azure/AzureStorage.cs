...
using MiniE_TicaretAPI.Application.Abstractions.Storage.Azure;

namespace MiniE_TicaretAPI.Infrastructure.Services.Storage.Azure
{
    public class AzureStorage :Storage, IAzureStorage
    {
        ...
        public async Task<List<(string fileName, string pathOrContainerName)>> UploadAsync(string containerName, IFormFileCollection files, string productName)
        {
            
            _blobContainerClient = _blobServiceClient.GetBlobContainerClient($"{containerName}-{productName.ToLower()}");//container yerini aldık.
            await _blobContainerClient.CreateIfNotExistsAsync();//eğer container yoksa oluşturuyoruz.
            await _blobContainerClient.SetAccessPolicyAsync(PublicAccessType.BlobContainer);//erişim izni verdik.
            List<(string fileName, string pathOrContainerName)> datas = new();
            foreach (IFormFile file in files)
            {
                string fileNewName = await FileRenameAsync(productName, file.FileName);
                BlobClient blobClient = _blobContainerClient.GetBlobClient(fileNewName);
                await blobClient.UploadAsync(file.OpenReadStream());//elimdeki dosyayı azure'a stream olarak gönderelim.
                datas.Add((fileNewName, $"{containerName}/{fileNewName}"));
